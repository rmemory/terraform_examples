...
resource "aws_subnet" "worker-subnet-public-1" {
  provider = aws.region-worker
  ...
  tags = {
    Name = "worker-subnet-public-1"
  }
}

# Private subnet #1 in us-west-2
resource "aws_subnet" "worker-subnet-private-1" {
  provider = aws.region-worker
  # availability_zone not specified
  vpc_id                  = aws_vpc.worker.id
...
resource "aws_route_table" "worker-internet-route" {
  provider = aws.region-worker
  vpc_id   = aws_vpc.worker.id
...
resource "aws_main_route_table_association" "worker-route-association" {
  provider       = aws.region-worker
  vpc_id         = aws_vpc.worker.id
  route_table_id = aws_route_table.worker-internet-route.id
}