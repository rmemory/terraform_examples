...
resource "aws_subnet" "primary_subnet_public_1" {
  provider                = aws.us-east-1
  ...
  tags = {
    Name = "primary_subnet_public_1"
  }
}

# Create subnet #2 in us-east-1
resource "aws_subnet" "primary_subnet_public_2" {
  provider                = aws.us-east-1
  availability_zone       = element(data.aws_availability_zones.master_azs.names, 1)
  vpc_id                  = aws_vpc.primary.id
  cidr_block              = "10.0.2.0/24"
  map_public_ip_on_launch = "true"

  tags = {
    Name = "primary_subnet_public_2"
  }
}

# Private subnet #1 in us-east-1
resource "aws_subnet" "primary_subnet_private_1" {
  provider                = aws.us-east-1
  availability_zone       = element(data.aws_availability_zones.master_azs.names, 2)
  vpc_id                  = aws_vpc.primary.id
  cidr_block              = "10.0.3.0/24"
  map_public_ip_on_launch = "false"

  tags = {
    Name = "primary_subnet_private_1"
  }
}
...
resource "aws_route_table" "primary_internet_route_tbl" {
  provider = aws.us-east-1
  vpc_id   = aws_vpc.primary.id
  ...
  tags = {
    Name = "primary_internet_route_tbl"
  }
}

#Overwrite default route table of VPC(Master) with our route table entries
resource "aws_main_route_table_association" "primary_route_association" {
  provider       = aws.us-east-1
  vpc_id         = aws_vpc.primary.id
  route_table_id = aws_route_table.primary_internet_route_tbl.id
}
